...
print("Importing autoencoder classifier using probability space adversarial perturbations, Ani's idea!")
...
def get_output(model, x, encoder, encoder_b, decoder_b, gradient_weight=1.0, perturb_size=0.1):
    ...
    perturbed_presoftmax_ = gaussian_corrupt(presoftmax_,perturb_size)

    hgrad = tf.gradients(tf.reduce_mean(tf.square(tf.stop_gradient(perturbed_presoftmax_) - presoftmax_)), [h_input_to_dae_])

    h_input_to_dae_ = h_input_to_dae_ - tf.stop_gradient(hgrad[0] * gradient_weight)
    output = tf.nn.tanh(tf.matmul(h_input_to_dae_, encoder[0]) + encoder_b[0])
    output_ = tf.nn.tanh(tf.matmul(output, tf.transpose(encoder[0])) + decoder_b[0])
    presoftmax_ = model.layers[2].fprop(output_)
