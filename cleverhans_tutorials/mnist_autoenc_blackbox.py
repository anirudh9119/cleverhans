...
def mnist_tutorial(train_start=0, train_end=60000, test_start=0,
    ...
    if clean_train:
        ...
        #create the substitute model and train  
        print("Training Substitute Model")
        sub_encoder, sub_encoder_b, sub_decoder_b, sub_autoencoder_params, sub_corrupt_prob = autoencoder(dimensions=[512, 128])
        sub_model = make_basic_fc(sub_encoder, sub_encoder_b, sub_decoder_b, sub_autoencoder_params)
        sub_preds,sub_hpre,sub_hpost = get_output(sub_model, x, sub_encoder, sub_encoder_b, sub_decoder_b, sub_autoencoder_params)
        sub_rec_cost = compute_rec_error(sub_hpre,sub_hpost)
        def evaluate():
            # Evaluate the accuracy of the MNIST model on legitimate test
            # examples
            eval_params = {'batch_size': batch_size}
            acc,rec_error = model_eval_2(
                sess, x, y, corrupt_prob, sub_preds, X_test, Y_test, args=eval_params, rec_cost=sub_rec_cost)
            report.clean_train_clean_eval = acc
            assert X_test.shape[0] == test_end - test_start, X_test.shape
            print('Sub Test accuracy on legitimate examples: %0.4f' % acc)
            print('Sub rec error on legitimate examples: %0.4f' % rec_error)
        model_train_2(sess, x, y, corrupt_prob, sub_preds, X_train, Y_train, evaluate=evaluate,rec_cost=sub_rec_cost,args=train_params, rng=rng)

        adv_x = create_adv_by_name(sub_model, x, attack_name, sess, 'mnist')
        ...
        print('Sub Test accuracy on adversarial examples: %0.4f\n' % acc)
        print('Sub rec error adv->adv on adversarial examples: %0.4f\n' % rec_error)
    ...
    if with_denoising:
        encoder_2, encoder_b_2, decoder_b_2, autoencoder_params_2, corrupt_prob_2 = autoencoder(dimensions=[512, 320])
        model_2 = make_basic_fc(encoder_2, encoder_b_2, decoder_b_2, autoencoder_params_2)
    ...
    if with_denoising == False:
        ...
        preds_2,hpreclean,hpostclean = get_output(model_2, x, encoder_2, encoder_b_2, decoder_b_2, autoencoder_params_2)
        cost_2 = compute_rec_error(hpreclean,hpostclean)
        if not backprop_through_attack:
            adv_x_2 = tf.stop_gradient(adv_x_2)
        preds_2_adv,hpreadv,hpostadv = get_output(model_2, adv_x_2, encoder_2, encoder_b_2, decoder_b_2, autoencoder_params_2)
    ...
    print("Evaluate adv-train model against Substitute Model")

    sub_adv_x = create_adv_by_name(sub_model, x, attack_name, sess, 'mnist')
    #sub_adv_x = x
    sub_adv_x = tf.reshape(sub_adv_x, [-1, 784])
    sub_preds_adv,sub_hpreadv,sub_hpostadv = get_output(model_2, sub_adv_x, encoder_2, encoder_b_2, decoder_b_2, autoencoder_params_2)
    sub_cost = compute_rec_error(sub_hpreadv,sub_hpostadv)
    # Evaluate the accuracy of the MNIST model on adversarial examples
    eval_par = {'batch_size': batch_size}
    acc,rec_error = model_eval_2(sess, x, y, corrupt_prob, sub_preds_adv, X_test, Y_test, rec_cost=sub_cost, args=eval_par)
    print('Sub Test accuracy on adversarial examples: %0.4f\n' % acc)
    print('Sub rec error adv->adv on adversarial examples: %0.4f\n' % rec_error)
    report.clean_train_adv_eval = acc
